...
class App():
    ...
    def nohave_url(self):
        ...
        self.label_intervalo = tk.Label(self.retangulo, self.configuracoes_label,  text='Intervalo:')
    ...
    def ListaSites(self):
        if self.cont != 1:
            self.label_sb = tk.Label(self.retangulo, font=self.fontebutton, text='Sites vulneráveis', bg=bg_retangulo)
    ...
    def Verificar_haveurl(self):
        ...
        for i in range(0, len(self.sites)):
                try:
                    ...
                    print('Não é possível analisar '+self.sites[i]+' :(')
                    i = i+1
